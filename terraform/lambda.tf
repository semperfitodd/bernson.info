module "lambda_function_linkedin" {
  ...
  description   = "${var.todd_name}-${local.environment} function to get LinkedIn articles"
  function_name = "${var.todd_name}-${local.environment}-linkedin"
  ...
  policy_statements = {
    ...
    secrets = {
      ...
      resources = [aws_secretsmanager_secret.linkedin_token.arn]
    }
  }
...
resource "aws_cloudwatch_event_rule" "daily_trigger" {
  name                = "${var.todd_name}-${local.environment}-linkedin-daily-trigger"
  description         = "Daily trigger for LinkedIn articles Lambda"
